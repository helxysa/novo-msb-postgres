...
export async function deleteAta(numero: string) {
  try {
    await api.delete(`/atas/${numero}`);
    return true;
  } catch (error) {
...
export async function updateAta(numero: string, data: any) {
  try {
    const response = await api.put(`/atas/${numero}`, data);
    return response.data;
  } catch (error: any) {
...
export async function createCandidato(
  ...
  try {
    ...
    if (error instanceof AxiosError) {
      ...
      throw new Error(
        error.response?.data?.message || "Erro ao criar candidatura"
      );
    }
    throw error;
  }
